...
class GetAllPostApi {
  ...
  Future<CommentPostModel> commentPosts(int i) async {
    ...
    try {
      ...
      map['post_id'] = viewPost.data[i].id.toString();
      map['caption'] = Constants.commentPost.caption.toString();
...
//  ViewAllProfileModel viewAllProfileModel;
  ...
  Future<void> followUser(int i) async {
    ...
    final response = await http.post(Uri.parse(Constants.baseUrl + "follow-user"),
        body: map, headers: header);
    final jsonResponse = json.decode(response.body);
    if (jsonResponse["status"].toString() != "Error") {
